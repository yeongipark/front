...
function StepAuthVerification({
    ...
    const handleDuplicateCheck = () => {
        axios.get(`http://43.203.69.159/api/v1/auth/register/check/${formData.userNickName}`).then((res) => {
            ...
            <div className="register_modal__section">
                ...
                <div className="register_modal__email">
                    <Input
                        ...
                        autocomplete={'off'}
                    />
                    {companyEmail !== '없음' && <span>{companyEmail ? '@' + companyEmail : ''}</span>}
                    <button className="auth_button" onClick={sendAuthCode} disabled={isActive}>
                        인증
                    </button>
